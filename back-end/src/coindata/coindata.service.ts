...
export async function fetchCoinMarketData() {
  ...
  for (let i = 0; i < coinMarketData.length; i++) {
    let coin = await Coin.updateOne(
      { id: coinMarketData[i].id },
      {
        id: coinMarketData[i].id,
        symbol: coinMarketData[i].symbol,
        currentMarketData: coinMarketData[i],
        currentPrice: {
          USD: coinMarketData[i].current_price,
          lastUpdated: now,
        },
      },
      { upsert: true },
...
export async function getCoinId(symbol: string) {
...
export async function getCoinMarketData(symbol: string) {
  let symbolLower = symbol.toLowerCase();
  ...
  if (!coinData) {
    ...
    if (!coinData) {
      throw `Failed to fetch coin market data for symbol '${symbol}'`;
    }
  }
...
export async function fetchCurrentPrice(coinId: string): Promise<number> {
  var tickerJson = await axios
    .get<ISimplePriceResponse>(
...
export async function getCoinDataDocument(symbol: string) {
  let symbolLower = symbol.toLowerCase();
...
export async function fetchAllCoinsMarketData(
  page: number = 1
...
export async function fetchCoinMarketDataPage(page: number = 1) {
